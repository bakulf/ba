...
int main()
{
	...
	err = Pa_Initialize();
	if( err != paNoError ) {
		std::cout <<   "PortAudio error1: " << Pa_GetErrorText( err ) << std::endl;
        }
	...
	if( err != paNoError )
		std::cout <<   "PortAudio error2: " << Pa_GetErrorText( err ) << std::endl;
	...
	err = Pa_Terminate();
	if( err != paNoError )
		std::cout <<  "PortAudio error3: "<< Pa_GetErrorText( err ) << std::endl;
